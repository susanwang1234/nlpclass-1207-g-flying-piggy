...
printable_map = dict([(char, index) for index, char in enumerate(string.printable)])
width = 100
...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size):
        ...
        # The second RNN
        self.secondRNN = SecondRNN(width * 3, hidden_dim)

    def forward(self, sentence, character_level):
        hidden_layer = self.secondRNN(character_level, torch.zeros(len(sentence), self.hidden_dim))
        embeds = torch.cat([self.word_embeddings(sentence), hidden_layer], 1)
...
class LSTMTagger:
    ...
    def argmax(self, seq):
        ...
        with torch.no_grad():
            ...
            tag_scores = self.model(inputs, character_level_repr(seq))
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                character_level = character_level_repr(sentence);

                # Step 3. Run our forward pass.
                tag_scores = self.model(sentence_in, character_level)

                # Step 4. Compute the loss, gradients, and update the parameters by
                #  calling optimizer.step()
                loss = loss_function(tag_scores, targets) * 5
                loss.backward()
                self.optimizer.step()
