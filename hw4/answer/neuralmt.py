...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        """
        seq, batch, dim = encoder_out.shape    # size: seq * bacth * dim
        scores = self.W_enc(encoder_out.permute(1, 0, 2)) + self.W_dec(decoder_hidden.permute(1, 0, 2)) # size: batch * seq * dim
        alpha = torch.nn.functional.softmax(self.V_att(torch.tanh(scores)), dim=1)                      # size: batch * seq * 1
        return alpha
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        context = (torch.sum(alpha.permute(1, 0, 2) * encoder_out, dim=0)).reshape(1, 1, dim)  # sum up each seq
...
class Node():
    def __init__(self, parent, log_prob, output, decoder_hidden):
        self.parent = parent
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, maxLen=None, eos_index=2):
        ...
        return beamDecoder(self.decoder, encoder_out, encoder_hidden,
                             maxLen, eos_index, self.params['tgtLexSize'])
